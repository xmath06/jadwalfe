<script lang="ts">
	...
	import MapelForm from '$lib/components/forms/MapelForm.svelte';
	...
	async function fetchData() {
		loading = true;
		try {
			const res = await api.get('/mata-pelajaran', { params: { page, limit, search } });
			data = res.data;
	...
	async function handleSave(formData) {
		try {
			...
			if (editItem) {
				await api.put(`/mata-pelajaran/${editItem.id_mapel}`, { ...payload, id_mapel: undefined });
				toastState.success('Mapel berhasil diperbarui');
			} else {
				payload.id_mapel = `MAP-${String(Date.now()).slice(-4)}`;
				await api.post('/mata-pelajaran', payload);
				toastState.success('Mapel berhasil ditambahkan');
			}
			showModal = false;
			fetchData();
		} catch (err) {
			toastState.error(err.message || 'Gagal menyimpan mapel');
		}
	}
	...
	async function handleDelete(id) {
		...
		try {
			...
			toastState.success('Mapel berhasil dihapus');
			fetchData();
		} catch (err) {
			toastState.error(err.message || 'Gagal menghapus mapel');
		}
	}
	...
	function downloadTemplate() {
		...
		const ws = XLSX.utils.aoa_to_sheet([
			['nama_mapel', 'kelompok', 'minimal_jp_per_pertemuan', 'wajib_jam_pagi'],
			['Contoh Mapel', 'Wajib', '2', 'TRUE', 'FALSE']
		]);
		ws['!cols'] = [{ wch: 25 }, { wch: 18 }, { wch: 24 }, { wch: 16 }, { wch: 18 }];
		...
		reader.onload = async (evt) => {
			try {
				...
				for (let i = 0; i < rows.length; i++) {
					const row = rows[i];
					try {
						await api.post('/mata-pelajaran', {
							id_mapel: `MAP-${String(Date.now()).slice(-4)}-${i}`,
							id_sekolah: '1',
							nama_mapel: row.nama_mapel,
							kelompok: row.kelompok || 'Wajib',
							minimal_jp_per_pertemuan: Number(row.minimal_jp_per_pertemuan) || 2,
							wajib_jam_pagi: String(row.wajib_jam_pagi).toUpperCase() === 'TRUE'
						});
						success++;
					} catch (e) {
				...
				if (errors.length > 0) {
					...
					toastState.success(`${success} mapel berhasil diimport`);
				}
				e.target.value = '';
				fetchData();
			} catch (err) {
...
<div class="p-6">
	<div class="flex items-center justify-between mb-6">
		<h1 class="text-xl font-bold text-slate-800">Data Mata Pelajaran</h1>
	...
	<div class="bg-white rounded-xl shadow-sm border border-slate-200">
		<div class="p-4 border-b border-slate-200 flex items-center gap-4">
			<div class="relative flex-1 max-w-sm">
				...
				<input type="text" value={search} oninput={onSearchInput} placeholder="Cari mapel..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
			</div>
			...
			<table class="w-full text-sm">
				<thead class="bg-slate-50 text-slate-600 text-xs uppercase">
					<tr>
						...
						<th class="text-center px-4 py-3 font-medium">Min JP</th>
						<th class="text-center px-4 py-3 font-medium">Wajib Pagi</th>
				...
				<tbody class="divide-y divide-slate-100">
					...
					{:else}
						{#each data as item (item.id_mapel)}
							<tr class="hover:bg-slate-50">
								<td class="px-4 py-3 font-mono text-xs text-slate-500">{item.id_mapel}</td>
								<td class="px-4 py-3 font-medium text-slate-800">{item.nama_mapel}</td>
								<td class="px-4 py-3 text-slate-600">{item.kelompok}</td>
								<td class="px-4 py-3 text-center font-mono text-slate-700">{item.minimal_jp_per_pertemuan}</td>
								<td class="px-4 py-3 text-center">
									{#if item.wajib_jam_pagi}
								...
								<td class="px-4 py-3">
									<div class="flex items-center justify-center gap-2">
										...
										<button onclick={() => handleDelete(item.id_mapel)} class="p-1.5 rounded text-red-600 hover:bg-red-50" title="Hapus">
		...
		<div class="px-4 py-3 border-t border-slate-200 flex items-center justify-between text-sm text-slate-600">
			<span>{totalItems > 0 ? `Menampilkan ${(page - 1) * limit + 1}-${Math.min(page * limit, totalItems)} dari ${totalItems} Mapel` : 'Tidak ada data'}</span>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onclick={() => showModal = false} onkeydown={(e) => e.key === 'Escape' && (showModal = false)} role="presentation">
		<div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4" onclick={(e) => e.stopPropagation()} onkeydown={() => {}} role="dialog" tabindex="-1">
			<div class="px-6 py-4 border-b border-slate-200">
				<h2 class="text-lg font-semibold text-slate-800">{editItem ? 'Edit Mapel' : 'Tambah Mapel'}</h2>
			</div>
			<div class="px-6 py-4">
				<MapelForm data={editItem} onSave={handleSave} onCancel={() => showModal = false} />
			</div>
		</div>
	</div>
{/if}
