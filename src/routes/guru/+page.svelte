<script lang="ts">
	...
	import GuruForm from '$lib/components/forms/GuruForm.svelte';
	...
	async function fetchData() {
		loading = true;
		try {
			const res = await api.get('/guru', { params: { page, limit, search } });
			data = res.data;
	...
	async function handleSave(formData) {
		try {
			...
			if (editItem) {
				await api.put(`/guru/${editItem.id_guru}`, { ...payload, id_guru: undefined });
				toastState.success('Guru berhasil diperbarui');
			} else {
				payload.id_guru = `GUR-${String(Date.now()).slice(-4)}`;
				await api.post('/guru', payload);
				toastState.success('Guru berhasil ditambahkan');
			}
			showModal = false;
			fetchData();
		} catch (err) {
	...
	async function handleDelete(id) {
		...
		try {
			...
			toastState.success('Guru berhasil dihapus');
			fetchData();
		} catch (err) {
			toastState.error(err.message || 'Gagal menghapus guru');
		}
	}
	...
	function downloadTemplate() {
		...
		const ws = XLSX.utils.aoa_to_sheet([
			['nama_guru', 'tanggal_lahir', 'status', 'gunakan_prioritas_pagi', 'gunakan_prioritas_kontinu'],
			['Contoh Nama', '1990-01-01', 'Aktif', 'TRUE', 'FALSE']
		]);
		ws['!cols'] = [{ wch: 30 }, { wch: 14 }, { wch: 12 }, { wch: 22 }, { wch: 24 }];
		...
		reader.onload = async (evt) => {
			try {
				...
				for (let i = 0; i < rows.length; i++) {
					...
					try {
						await api.post('/guru', {
							id_guru: `GUR-${String(Date.now()).slice(-4)}-${i}`,
							id_sekolah: '1',
							nama_guru: row.nama_guru,
							tanggal_lahir: row.tanggal_lahir,
							status: row.status || 'Aktif',
							gunakan_prioritas_pagi: String(row.gunakan_prioritas_pagi).toUpperCase() === 'TRUE',
							gunakan_prioritas_kontinu: String(row.gunakan_prioritas_kontinu).toUpperCase() === 'TRUE'
						});
						success++;
					} catch (e) {
				...
				if (errors.length > 0) {
					...
					toastState.success(`${success} guru berhasil diimport`);
				}
				e.target.value = '';
				fetchData();
			} catch (err) {
...
<div class="p-6">
	<div class="flex items-center justify-between mb-6">
		<h1 class="text-xl font-bold text-slate-800">Data Guru</h1>
	...
	<div class="bg-white rounded-xl shadow-sm border border-slate-200">
		<div class="p-4 border-b border-slate-200 flex items-center gap-4">
			<div class="relative flex-1 max-w-sm">
				...
				<input type="text" value={search} oninput={onSearchInput} placeholder="Cari guru..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
			</div>
			...
			<table class="w-full text-sm">
				<thead class="bg-slate-50 text-slate-600 text-xs uppercase">
					<tr>
						...
						<th class="text-left px-4 py-3 font-medium">Tanggal Lahir</th>
						<th class="text-left px-4 py-3 font-medium">Status</th>
						<th class="text-center px-4 py-3 font-medium">Prioritas Pagi</th>
						<th class="text-center px-4 py-3 font-medium">Prioritas Kontinu</th>
						<th class="text-center px-4 py-3 font-medium">Aksi</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-slate-100">
					{#if loading}
						<tr><td colspan="7" class="px-4 py-8 text-center text-slate-500">Memuat data...</td></tr>
					{:else if data.length === 0}
						<tr><td colspan="7" class="px-4 py-8 text-center text-slate-500">Tidak ada data</td></tr>
					{:else}
						{#each data as item (item.id_guru)}
							<tr class="hover:bg-slate-50">
								<td class="px-4 py-3 font-mono text-xs text-slate-500">{item.id_guru}</td>
								<td class="px-4 py-3 font-medium text-slate-800">{item.nama_guru}</td>
								<td class="px-4 py-3 text-slate-600">{item.tanggal_lahir?.toString().slice(0, 10)}</td>
								<td class="px-4 py-3">
									<span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium {item.status === 'Aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}">{item.status}</span>
								</td>
								<td class="px-4 py-3 text-center">
									{#if item.gunakan_prioritas_pagi}
								...
								<td class="px-4 py-3 text-center">
									{#if item.gunakan_prioritas_kontinu}
										<span class="text-green-600">✓</span>
									{:else}
										<span class="text-slate-300">—</span>
									{/if}
								...
								<td class="px-4 py-3">
									<div class="flex items-center justify-center gap-2">
										...
										<button onclick={() => handleDelete(item.id_guru)} class="p-1.5 rounded text-red-600 hover:bg-red-50" title="Hapus">
		...
		<div class="px-4 py-3 border-t border-slate-200 flex items-center justify-between text-sm text-slate-600">
			<span>{totalItems > 0 ? `Menampilkan ${(page - 1) * limit + 1}-${Math.min(page * limit, totalItems)} dari ${totalItems} Guru` : 'Tidak ada data'}</span>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onclick={() => showModal = false} onkeydown={(e) => e.key === 'Escape' && (showModal = false)} role="presentation">
		<div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4" onclick={(e) => e.stopPropagation()} onkeydown={() => {}} role="dialog" tabindex="-1">
			<div class="px-6 py-4 border-b border-slate-200">
				<h2 class="text-lg font-semibold text-slate-800">{editItem ? 'Edit Guru' : 'Tambah Guru'}</h2>
			</div>
			<div class="px-6 py-4">
				<GuruForm data={editItem} onSave={handleSave} onCancel={() => showModal = false} />
			</div>
		</div>
	</div>
{/if}
